  ...
  return (
    <main className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-800/90 border border-slate-700 rounded-2xl p-8 space-y-6 shadow-2xl">
        ...
        {successMessage && (
          <div className="p-3 bg-emerald-900/40 border border-emerald-700 rounded-xl text-emerald-200 text-xs text-left">
            ✅ {successMessage}
          </div>
        )}
        ...
        {step === "OTP" && (
          <form onSubmit={handleOtpSubmit} className="space-y-4">
            <div className="space-y-1.5 text-left">
              <label className="text-xs font-semibold text-slate-400">
                Code sent to <span className="text-white font-medium">{authEmail}</span>
              </label>
              <input
                type="text"
                required
